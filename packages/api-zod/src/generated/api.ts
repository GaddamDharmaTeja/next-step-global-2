/**
 * Generated by orval v8.5.3 🍺
 * Do not edit manually.
 * Api
 * NextStep Global Educational Services API
 * OpenAPI spec version: 0.1.0
 */
import * as zod from "zod";

/**
 * @summary Health check
 */
export const HealthCheckResponse = zod.object({
  status: zod.string(),
});

/**
 * @summary Get current user profile
 */
export const GetMyProfileResponse = zod.object({
  id: zod.string(),
  clerkId: zod.string(),
  email: zod.string(),
  name: zod.string().nullish(),
  phone: zod.string().nullish(),
  role: zod.enum(["user", "admin", "owner"]),
  createdAt: zod.string(),
});

/**
 * @summary List all users (admin)
 */
export const ListUsersResponseItem = zod.object({
  id: zod.string(),
  clerkId: zod.string(),
  email: zod.string(),
  name: zod.string().nullish(),
  phone: zod.string().nullish(),
  role: zod.enum(["user", "admin", "owner"]),
  createdAt: zod.string(),
});
export const ListUsersResponse = zod.array(ListUsersResponseItem);

/**
 * @summary Update user role (admin)
 */
export const UpdateUserRoleParams = zod.object({
  userId: zod.coerce.string(),
});

export const UpdateUserRoleBody = zod.object({
  role: zod.enum(["user", "admin", "owner"]),
});

export const UpdateUserRoleResponse = zod.object({
  id: zod.string(),
  clerkId: zod.string(),
  email: zod.string(),
  name: zod.string().nullish(),
  phone: zod.string().nullish(),
  role: zod.enum(["user", "admin", "owner"]),
  createdAt: zod.string(),
});

/**
 * @summary List all inquiries (admin)
 */
export const ListInquiriesResponseItem = zod.object({
  id: zod.number(),
  name: zod.string(),
  email: zod.string(),
  phone: zod.string().nullish(),
  whatsapp: zod.string().nullish(),
  subject: zod.string(),
  message: zod.string(),
  status: zod.enum(["pending", "contacted", "resolved"]),
  createdAt: zod.string(),
});
export const ListInquiriesResponse = zod.array(ListInquiriesResponseItem);

/**
 * @summary Submit a new inquiry
 */
export const CreateInquiryBody = zod.object({
  name: zod.string(),
  email: zod.string(),
  phone: zod.string().optional(),
  whatsapp: zod.string().optional(),
  subject: zod.string(),
  message: zod.string(),
});

/**
 * @summary Get inquiry by ID
 */
export const GetInquiryParams = zod.object({
  inquiryId: zod.coerce.number(),
});

export const GetInquiryResponse = zod.object({
  id: zod.number(),
  name: zod.string(),
  email: zod.string(),
  phone: zod.string().nullish(),
  whatsapp: zod.string().nullish(),
  subject: zod.string(),
  message: zod.string(),
  status: zod.enum(["pending", "contacted", "resolved"]),
  createdAt: zod.string(),
});

/**
 * @summary Update inquiry status (admin)
 */
export const UpdateInquiryParams = zod.object({
  inquiryId: zod.coerce.number(),
});

export const UpdateInquiryBody = zod.object({
  status: zod.enum(["pending", "contacted", "resolved"]).optional(),
  notes: zod.string().optional(),
});

export const UpdateInquiryResponse = zod.object({
  id: zod.number(),
  name: zod.string(),
  email: zod.string(),
  phone: zod.string().nullish(),
  whatsapp: zod.string().nullish(),
  subject: zod.string(),
  message: zod.string(),
  status: zod.enum(["pending", "contacted", "resolved"]),
  createdAt: zod.string(),
});

/**
 * @summary Delete inquiry (admin)
 */
export const DeleteInquiryParams = zod.object({
  inquiryId: zod.coerce.number(),
});

/**
 * @summary List all programs/courses
 */
export const ListProgramsResponseItem = zod.object({
  id: zod.number(),
  title: zod.string(),
  description: zod.string(),
  country: zod.string(),
  duration: zod.string(),
  imageUrl: zod.string().nullish(),
  tuitionFee: zod.string().nullish(),
  intakeMonths: zod.array(zod.string()),
  eligibility: zod.string().nullish(),
  englishRequirement: zod.string().nullish(),
  applicationDeadline: zod.string().nullish(),
  scholarshipAvailable: zod.boolean(),
  careerOutcomes: zod.array(zod.string()),
  featured: zod.boolean(),
  createdAt: zod.string(),
});
export const ListProgramsResponse = zod.array(ListProgramsResponseItem);

/**
 * @summary Create a new program (admin)
 */
export const CreateProgramBody = zod.object({
  title: zod.string(),
  description: zod.string(),
  country: zod.string(),
  duration: zod.string(),
  imageUrl: zod.string().optional(),
  tuitionFee: zod.string().optional(),
  intakeMonths: zod.array(zod.string()).optional(),
  eligibility: zod.string().optional(),
  englishRequirement: zod.string().optional(),
  applicationDeadline: zod.string().optional(),
  scholarshipAvailable: zod.boolean().optional(),
  careerOutcomes: zod.array(zod.string()).optional(),
  featured: zod.boolean().optional(),
});

/**
 * @summary Get program by ID
 */
export const GetProgramParams = zod.object({
  programId: zod.coerce.number(),
});

export const GetProgramResponse = zod.object({
  id: zod.number(),
  title: zod.string(),
  description: zod.string(),
  country: zod.string(),
  duration: zod.string(),
  imageUrl: zod.string().nullish(),
  tuitionFee: zod.string().nullish(),
  intakeMonths: zod.array(zod.string()),
  eligibility: zod.string().nullish(),
  englishRequirement: zod.string().nullish(),
  applicationDeadline: zod.string().nullish(),
  scholarshipAvailable: zod.boolean(),
  careerOutcomes: zod.array(zod.string()),
  featured: zod.boolean(),
  createdAt: zod.string(),
});

/**
 * @summary Update program (admin)
 */
export const UpdateProgramParams = zod.object({
  programId: zod.coerce.number(),
});

export const UpdateProgramBody = zod.object({
  title: zod.string().optional(),
  description: zod.string().optional(),
  country: zod.string().optional(),
  duration: zod.string().optional(),
  imageUrl: zod.string().optional(),
  tuitionFee: zod.string().optional(),
  intakeMonths: zod.array(zod.string()).optional(),
  eligibility: zod.string().optional(),
  englishRequirement: zod.string().optional(),
  applicationDeadline: zod.string().optional(),
  scholarshipAvailable: zod.boolean().optional(),
  careerOutcomes: zod.array(zod.string()).optional(),
  featured: zod.boolean().optional(),
});

export const UpdateProgramResponse = zod.object({
  id: zod.number(),
  title: zod.string(),
  description: zod.string(),
  country: zod.string(),
  duration: zod.string(),
  imageUrl: zod.string().nullish(),
  tuitionFee: zod.string().nullish(),
  intakeMonths: zod.array(zod.string()),
  eligibility: zod.string().nullish(),
  englishRequirement: zod.string().nullish(),
  applicationDeadline: zod.string().nullish(),
  scholarshipAvailable: zod.boolean(),
  careerOutcomes: zod.array(zod.string()),
  featured: zod.boolean(),
  createdAt: zod.string(),
});

/**
 * @summary Delete program (admin)
 */
export const DeleteProgramParams = zod.object({
  programId: zod.coerce.number(),
});

/**
 * @summary List all gallery images
 */
export const ListGalleryImagesResponseItem = zod.object({
  id: zod.number(),
  url: zod.string(),
  caption: zod.string().nullish(),
  category: zod.string().nullish(),
  sortOrder: zod.number(),
  createdAt: zod.string(),
});
export const ListGalleryImagesResponse = zod.array(
  ListGalleryImagesResponseItem,
);

/**
 * @summary Add gallery image (admin)
 */
export const CreateGalleryImageBody = zod.object({
  url: zod.string(),
  caption: zod.string().optional(),
  category: zod.string().optional(),
  sortOrder: zod.number().optional(),
});

/**
 * @summary Update gallery image (admin)
 */
export const UpdateGalleryImageParams = zod.object({
  imageId: zod.coerce.number(),
});

export const UpdateGalleryImageBody = zod.object({
  url: zod.string().optional(),
  caption: zod.string().optional(),
  category: zod.string().optional(),
  sortOrder: zod.number().optional(),
});

export const UpdateGalleryImageResponse = zod.object({
  id: zod.number(),
  url: zod.string(),
  caption: zod.string().nullish(),
  category: zod.string().nullish(),
  sortOrder: zod.number(),
  createdAt: zod.string(),
});

/**
 * @summary Delete gallery image (admin)
 */
export const DeleteGalleryImageParams = zod.object({
  imageId: zod.coerce.number(),
});

/**
 * @summary List all testimonials
 */
export const ListTestimonialsResponseItem = zod.object({
  id: zod.number(),
  studentName: zod.string(),
  country: zod.string(),
  program: zod.string().nullish(),
  message: zod.string(),
  avatarUrl: zod.string().nullish(),
  rating: zod.number(),
  featured: zod.boolean(),
  createdAt: zod.string(),
});
export const ListTestimonialsResponse = zod.array(ListTestimonialsResponseItem);

/**
 * @summary Create testimonial (admin)
 */
export const CreateTestimonialBody = zod.object({
  studentName: zod.string(),
  country: zod.string(),
  program: zod.string().optional(),
  message: zod.string(),
  avatarUrl: zod.string().optional(),
  rating: zod.number(),
  featured: zod.boolean().optional(),
});

/**
 * @summary Update testimonial (admin)
 */
export const UpdateTestimonialParams = zod.object({
  testimonialId: zod.coerce.number(),
});

export const UpdateTestimonialBody = zod.object({
  studentName: zod.string().optional(),
  country: zod.string().optional(),
  program: zod.string().optional(),
  message: zod.string().optional(),
  avatarUrl: zod.string().optional(),
  rating: zod.number().optional(),
  featured: zod.boolean().optional(),
});

export const UpdateTestimonialResponse = zod.object({
  id: zod.number(),
  studentName: zod.string(),
  country: zod.string(),
  program: zod.string().nullish(),
  message: zod.string(),
  avatarUrl: zod.string().nullish(),
  rating: zod.number(),
  featured: zod.boolean(),
  createdAt: zod.string(),
});

/**
 * @summary Delete testimonial (admin)
 */
export const DeleteTestimonialParams = zod.object({
  testimonialId: zod.coerce.number(),
});

/**
 * @summary Send email/WhatsApp notification (admin)
 */
export const SendNotificationBody = zod.object({
  channel: zod.enum(["email", "whatsapp", "both"]),
  recipientEmail: zod.string().nullish(),
  recipientPhone: zod.string().nullish(),
  subject: zod.string(),
  message: zod.string(),
});

export const SendNotificationResponse = zod.object({
  success: zod.boolean(),
  emailSent: zod.boolean(),
  whatsappSent: zod.boolean(),
  message: zod.string(),
});

/**
 * @summary Get admin dashboard statistics
 */
export const GetAdminStatsResponse = zod.object({
  totalUsers: zod.number(),
  totalInquiries: zod.number(),
  pendingInquiries: zod.number(),
  totalPrograms: zod.number(),
  totalGalleryImages: zod.number(),
  totalTestimonials: zod.number(),
  recentInquiries: zod.array(
    zod.object({
      id: zod.number(),
      name: zod.string(),
      email: zod.string(),
      phone: zod.string().nullish(),
      whatsapp: zod.string().nullish(),
      subject: zod.string(),
      message: zod.string(),
      status: zod.enum(["pending", "contacted", "resolved"]),
      createdAt: zod.string(),
    }),
  ),
});
