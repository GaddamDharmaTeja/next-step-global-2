...
router.patch("/", requireAdmin, async (req, res): Promise<void> => {
  const updated = await updateStore((store) => {
    store.siteContent = {
      ...
      brandLogoUrl: cleanString(req.body?.brandLogoUrl, store.siteContent.brandLogoUrl || "") || null,
      heroBackgroundImage: cleanString(req.body?.heroBackgroundImage, store.siteContent.heroBackgroundImage || "") || null,
      heroRightImage: cleanString(req.body?.heroRightImage, store.siteContent.heroRightImage || "") || null,
